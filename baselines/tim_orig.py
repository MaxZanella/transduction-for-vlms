...
class TIM(object):
    ...
    def get_logits(self, samples):
        ...
        logits = self.temp * (samples.matmul(self.weights.transpose(0, 1)) \
    ...
    def init_weights(self, support, query, y_s, y_q):
        ...
        weights = one_hot.transpose(0, 1).matmul(support)
    ...
    def compute_lambda(self, support, query, y_s):
        """
        inputs:
            support : torch.Tensor of shape [s_shot, feature_dim]
            query : torch.Tensor of shape [q_shot, feature_dim]
            y_s : torch.Tensor of shape [s_shot]

        updates :
            self.loss_weights[0] : Scalar
        """
        self.N_s, self.N_q = support.size(0), query.size(0)
    ...
    def record_info(self, new_time, support, query, y_s, y_q):
        ...
        self.test_acc.append((preds_q == y_q).float().mean(0, keepdim=True))
